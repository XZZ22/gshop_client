...
2、获取商家信息info
 ...
 1、CartControl组件
 2、Food组件
  注意点：父组件中调用子组件的方法，利用ref进行标识
 3、ShopCart组件 --- 购物车
  注意点：
    （1）定义新的状态state，避免三层数据遍历时候性能差
    （2）注意功能性bug，底部的菜单列表实际上在下面
    （3）注意：底部的出现与否受两部分的控制，定义成计算属性
    （4）滑动：nextTick，
        注意：点击的时候，点击一次会调用一次BScroll对象的创建，利用滑动库的refresh
            v-if和v-show在滑动库中创建的区别
    （5）单例对象：
        创建前：判断不存在  创建后：保存
    （6）清空：两个数组
    （7）过渡：购物车和背景都加过渡
